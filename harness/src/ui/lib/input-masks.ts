...
export function formatDocumentBrInput(value: string): string {
...
export function formatCepBrInput(value: string): string {
  const digits = value.replace(/\D/g, "").slice(0, 8);
...
export function applyInputMask(type: string, value: string): string {
  switch (type) {
    ...
    case "document":
      return formatDocumentBrInput(value);
    case "cep":
      return formatCepBrInput(value);
    default:
      return value;
  }
}
